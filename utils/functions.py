...
def draw_bounding_boxess(image_path, annotation_path, output_path, **kwargs): # Argumen lama tidak relevan lagi
    ...
    try:
        ...
        # Jika tidak ada file anotasi (tidak ada spot yang terdeteksi), tangani dengan baik
        annotations = []
    ...
    for annotation in annotations:
        ...
        # Ubah koordinat dari normalisasi ke piksel
        ...
        color = (0, 0, 0) # Warna default (hitam)
        
        # Tentukan warna berdasarkan class_id dari dataset.yaml baru Anda
    ...
    # Tambahkan legenda/teks statistik ke gambar
    ...
    def draw_text_with_background(img, text, position, font, scale, text_color, bg_color, alpha=0.6, thickness=2, padding=10):
        ...
        x, y = position
        # Gambar rectangle sebagai latar belakang di overlay
        cv2.rectangle(
            overlay,
        ...
        # Gabungkan overlay ke image utama dengan transparansi
        cv2.addWeighted(overlay, alpha, img, 1 - alpha, 0, img)

        # Gambar teks di atas rectangle (langsung ke img)
...
# Parameter teks
    ...
    scale = 1
    text_color = (0, 0, 0)
    bg_color = (200, 200, 200)  # Abu-abu terang
    thickness = 2
    alpha = 0.6

# Gambar setiap teks dengan background transparan
...
def process_labels_baru(data_path, new_labels_folder):
    """
    Fungsi sederhana untuk memvalidasi dan mengembalikan path ke folder
    label hasil prediksi model. Tidak ada proses penyalinan/penggabungan.
    """
    print(f"Menggunakan label langsung dari folder prediksi: {new_labels_folder}")
    
    # Validasi sederhana untuk memastikan folder prediksi ada
    if not os.path.exists(new_labels_folder):
        # Jika tidak ada, kita tidak bisa melanjutkan.
        raise FileNotFoundError(f"Folder label prediksi tidak ditemukan di: {new_labels_folder}")
        
    # Langsung kembalikan path ke folder prediksi model
    return new_labels_folder

# Versi bisa dipake
def process_imagess(data_path: str, output_folder: str, model: str = ''): # DIUBAH: Hapus threshold & highlighted_cars
    processed_images = 0
    ...
    for image_file in os.listdir(images_folder):
        ...
        # TAMBAHKAN PENGECEKAN INI SEBELUM MEMPROSES
        if os.path.exists(annotation_path):
            # HANYA proses gambar jika file anotasinya ada
            results = draw_bounding_boxess(image_path, annotation_path, output_images_folder)
            processed_images += 1
            results_df = pd.concat([results_df, results], ignore_index=True)
        else:
            # Jika tidak ada, beri pemberitahuan agar Anda tahu
            print(f"⚠️ No detections for {image_file}, skipping visualization.")
            # Opsi: Anda bisa memilih untuk tetap menyalin gambar aslinya ke output jika mau
            # import shutil
...
def process_images(data_path: str, output_folder: str, threshold: float = 0.4, highlighted_cars: bool = True, model: str = ''):
    ...
    if model != '':
        ...
        try: 
            print(f'Using labels from {new_labels_folder}')
            labels_folder = process_labels_baru(data_path, new_labels_folder) # process labels
